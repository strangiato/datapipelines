...
def get_last_image(bucket_name):
    ...
    bucket_table = {
        bucket_base_name: "images_uploaded",
        bucket_base_name + "-processed": "images_processed",
        bucket_base_name + "-anonymized": "images_anonymized",
    }
    try:
...
def last_image_small(bucket_name):
    ...
    if image_name != "":
        html = LOCATION_TEMPLATE_SMALL.substitute(
            service_point=service_point, bucket_name=bucket_name, image_name=image_name
        )
    else:
...
def last_image_big(bucket_name):
    image_name = get_last_image(bucket_name)
    if image_name != "":
        html = LOCATION_TEMPLATE_BIG.substitute(
            service_point=service_point, bucket_name=bucket_name, image_name=image_name
        )
    else:
        html = '<h2 style="font-family: Roboto,Helvetica Neue,Arial,sans-serif;text-align: center; color: white;font-size: 15px;font-weight: 400;">No image to show</h2>'
