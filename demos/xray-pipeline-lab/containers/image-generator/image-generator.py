...
pneumonia_images = []
for image in s3sourceclient.list_objects(
    Bucket=bucket_source_name, Prefix="PNEUMONIA/"
)["Contents"]:
    pneumonia_images.append(image["Key"])
normal_images = []
for image in s3sourceclient.list_objects(Bucket=bucket_source_name, Prefix="NORMAL/")[
    "Contents"
]:
...
while (
    seconds_wait != 0
):  # This allows the container to keep running but not send any image if parameter is set to 0
